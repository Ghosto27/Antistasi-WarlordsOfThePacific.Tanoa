...
/*
_tipoGrupo = if (_lado == malos) then {if (!_esFIA) then {selectRandom gruposNATOSquad} else {FIASquad}} else {selectRandom gruposCSATSquad};

while {(spawner getVariable _marcador!=2) and (_cuenta < _tam) and (_cuentaTotal + 8 <= _solMax)} do
	{
	if ((diag_fps > minimoFPS) or (_cuenta == 0)) then
		{
		//_tipogrupo = gruposAAFmid call BIS_fnc_selectRandom;
		_grupo = [_posicion,_lado, _tipoGrupo] call BIS_Fnc_spawnGroup;
		sleep 1;
		_stance = "RANDOM";
