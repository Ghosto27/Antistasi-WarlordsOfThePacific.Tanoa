...
while {(time < _bleedOut) and (damage _unit > 0.25) and (alive _unit) /*and (lifeState _unit == "INCAPACITATED")*/ and (!(_unit getVariable ["respawning",false]))} do
	{
...
if (_isPlayer) then
	...
	if (isMultiplayer) then {[_unit,"remove"] remoteExec ["flagaction",0,_unit]};
	}
else
	{
	_unit stop false;
